...
function useAvailableActionsAndTriggers() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/api/v1/trigger/availabletrigger", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      })
      .then((x) => setAvailableTriggers(x.data.trigger));

    axios
      .get("http://localhost:3001/api/v1/action/availableaction", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      })
      .then((x) => setAvailableActions(x.data.availableaction));
  }, []);
          ...
          <PrimaryButton
            onClick={async () => {
              if (!selectedTrigger?.id) {
                setError("Please select a trigger first");
                return;
              }
              ...
              try {
                const response = await axios.post(
                  "http://localhost:3001/api/v1/zap/createzap",
                  {
