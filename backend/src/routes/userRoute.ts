...
router.post("/signup", async (req, res): Promise<any> => {
  try {
    ...
    const verification_link = `http://localhost:3001/api/v1/user/verify/${user_id}`;
    const body = `  Hi ${name},
        Welcome to the workflow automation,  
        click on the following link to verify
                    ${verification_link}
        `;
...
router.get("/verify/:userId", async (req, res): Promise<any> => {
